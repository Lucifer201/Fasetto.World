...
namespace Fasetto.World.Core
{
    ...
    /// For example, only allowing on task to access some data at a time, like the old
    /// asynchronous locks
    /// 
    /// 
    /// This awaiter uses the safer semaphore to prevent any chances of a deadlock
    /// </summary>
    public static class AsyncAwaiter
    {
        ...
        /// <param name="maxAccessCount">If this is the first call, sets the maximum number of tasks that can access
        /// this task before it waiting</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <param name="key">The key to await</param>
        /// <param name="task">The task to perform inside of the semaphore</param>
        /// <param name="maxAccessCount">If this is the first call, sets the maximum number of tasks that can access
        /// this task before it waiting</param>
        /// <returns></returns>
        public static async Task AwaitAsync(string key, Func<Task> task, int maxAccessCount = 1)
        {
            ...
            catch (Exception ex)
            {
                ...
                IoC.Logger.Log($"Crash in {nameof(AwaitAsync)}. {error}", LogLevel.Debug);
